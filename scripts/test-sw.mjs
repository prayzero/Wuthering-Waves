...
const shell = stores.get('wuwa-planner-v17');
...
assert.deepEqual(deleted, ['wuwa-planner-v14', 'wuwa-planner-v15', 'wuwa-planner-v16'], '자체 구버전 캐시만 삭제해야 합니다.');
...
assert(writes.some(write => write.cache === 'wuwa-planner-v17' && write.request === cssRequest.url),
  '같은 출처 정상 응답을 현재 셸 캐시에 저장해야 합니다.');

const cachedJs = { ok: true, type: 'basic', cached: true };
stores.get('wuwa-planner-v17').set('https://example.test/js/app.js', cachedJs);
